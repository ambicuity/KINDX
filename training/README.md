...
# KINDX Query Expansion Fine-Tuning

Train small language models to expand search queries for [QMD](https://github.com/ambicuity/KINDX)'s hybrid retrieval pipeline.
...
uv run eval.py ambicuity/kindx-query-expansion-1.7B
...
uv run convert_gguf.py --base Qwen/Qwen3-1.7B \
                       --sft ambicuity/kindx-query-expansion-1.7B-sft \
                       --grpo ambicuity/kindx-query-expansion-1.7B-grpo \
                       --output ambicuity/kindx-query-expansion-1.7B-gguf
```

### Using with Ollama

```bash
huggingface-cli download ambicuity/kindx-query-expansion-1.7B-gguf \
    kindx-query-expansion-1.7B-q4_k_m.gguf --local-dir .

echo 'FROM ./kindx-query-expansion-1.7B-q4_k_m.gguf' > Modelfile

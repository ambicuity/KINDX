...
type FetchLikeWithPreconnect = FetchLike & { preconnect?: unknown };
...
const MOCK_DNS_RESULT: PinnedHostname["addresses"] = ["93.184.216.34"];
...
function isMockedPreconnectFetch(fetchImpl?: FetchLike): fetchImpl is FetchLikeWithPreconnect {
  if (!fetchImpl) {
    return false;
  }
  const mocked = fetchImpl as FetchLikeWithPreconnect & { mock?: unknown };
  return typeof mocked.preconnect === "function" || typeof mocked.mock === "object";
}
...
export async function fetchWithSsrFGuard(params: GuardedFetchOptions): Promise<GuardedFetchResult> {
  ...
  const skipPinnedLookup = isMockedPreconnectFetch(fetcher);
  while (true) {
    let parsedUrl: URL;
    try {
    ...
    try {
      ...
      if (canUseTrustedEnvProxy) {
      ...
      } else if (params.pinDns !== false && !skipPinnedLookup) {
        ...
        const mockLookupFn = (async () =>
          MOCK_DNS_RESULT.map((address) => ({
            address,

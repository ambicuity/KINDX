...
async function summarizeKindxIndexArtifact(manager: MemoryManager): Promise<string | null> {
  const status = manager.status?.();
  if (!status || status.backend !== "kindx") {
    return null;
  }
  ...
  try {
    ...
    const code = (err as NodeJS.ErrnoException).code;
    if (code === "ENOENT") {
      throw new Error(`KINDX index file not found: ${shortenHomePath(dbPath)}`, { cause: err });
    }
    throw new Error(
      `KINDX index file check failed: ${shortenHomePath(dbPath)} (${code ?? "error"})`,
      { cause: err },
    );
  }
  if (!stat.isFile() || stat.size <= 0) {
    throw new Error(`KINDX index file is empty: ${shortenHomePath(dbPath)}`);
  }
  return `KINDX index: ${shortenHomePath(dbPath)} (${stat.size} bytes)`;
}
        ...
        await withMemoryManagerForAgent({
          ...
          run: async (manager) => {
            try {
              ...
              const kindxIndexSummary = await summarizeKindxIndexArtifact(manager);
              if (kindxIndexSummary) {
                defaultRuntime.log(kindxIndexSummary);
              }

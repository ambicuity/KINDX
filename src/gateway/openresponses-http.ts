...
import { recordDirectUsage } from "../../../engine/ai-usage.js";
...
function extractUsageFromResult(result: unknown): Usage & { cacheRead: number } {
  ...
  const u = usage as { input?: number; output?: number; cacheRead?: number; cacheWrite?: number; total?: number } | undefined;
  return {
    ...toUsage(u),
...
export async function handleOpenResponsesHttpRequest(
  ...
  if (!stream) {
    try {
      ...
      try {
        ...
        logWarn(`openresponses: failed to record usage: ${String(err)}`);
      }
  ...
  let closed = false;
  let unsubscribe = () => {};
  let finalUsage: (Usage & { cacheRead: number }) | undefined;
      ...
      try {
        ...
        recordDirectUsage(db, {
          ...
          usage: {
            ...
            cached_tokens: finalUsage.cacheRead,
          },
          durationMs: 0,
          status: "success",
        });

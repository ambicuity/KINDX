...
export async function startCanvasHost(opts: CanvasHostServerOpts): Promise<CanvasHostServer> {
  if (isDisabledByEnv() && opts.allowInTests !== true) {
    return { port: 0, host: "", rootDir: "", close: async () => {} };
  }
  ...
  const requestedHost = opts.listenHost?.trim();
  const listenHosts = requestedHost ? [requestedHost] : ["127.0.0.1", "localhost", "0.0.0.0"];
  ...
  let boundHost = listenHosts[0] ?? "127.0.0.1";
  ...
  opts.runtime.log(
    `canvas host listening on http://${boundHost}:${boundPort} (root ${handler.rootDir})`,
  );

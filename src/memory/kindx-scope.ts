...
type ParsedKindxSessionScope = {
  channel?: string;
  chatType?: "channel" | "group" | "direct";
  normalizedKey?: string;
};

export function isKindxScopeAllowed(
  scope: ResolvedKindxConfig["scope"],
  sessionKey?: string,
): boolean {
  if (!scope) {
    return true;
  }
  const parsed = parseKindxSessionScope(sessionKey);
...
export function deriveKindxScopeChannel(key?: string): string | undefined {
  return parseKindxSessionScope(key).channel;
}

export function deriveKindxScopeChatType(key?: string): "channel" | "group" | "direct" | undefined {
  return parseKindxSessionScope(key).chatType;
}

function parseKindxSessionScope(key?: string): ParsedKindxSessionScope {
  const normalized = normalizeKindxSessionKey(key);
  if (!normalized) {
    return {};
  }
  const parts = normalized.split(":").filter(Boolean);
  let chatType: ParsedKindxSessionScope["chatType"];
  if (
    parts.length >= 2 &&
...
function normalizeKindxSessionKey(key?: string): string | undefined {
  if (!key) {
    return undefined;
  }

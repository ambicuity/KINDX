...
const createKindxManagerMock = vi.mocked(KindxMemoryManager.create);
...
function createKindxCfg(agentId: string): OpenClawConfig {
  return {
...
async function createFailedKindxSearchHarness(params: { agentId: string; errorMessage: string }) {
  const cfg = createKindxCfg(params.agentId);
...
beforeEach(async () => {
  ...
  createKindxManagerMock.mockClear();
});

describe("getMemorySearchManager caching", () => {
  it("reuses the same KINDX manager instance for repeated calls", async () => {
    const cfg = createKindxCfg("main");
    ...
    expect(createKindxManagerMock).toHaveBeenCalledTimes(1);
  });

  it("evicts failed kindx wrapper so next call retries kindx", async () => {
    const retryAgentId = "retry-agent";
    const {
      cfg,
      manager: firstManager,
      firstResult: first,
    } = await createFailedKindxSearchHarness({
      agentId: retryAgentId,
      errorMessage: "kindx query failed",
    });
    ...
    // eslint-disable-next-line @typescript-eslint/unbound-method
    expect(createKindxManagerMock).toHaveBeenCalledTimes(2);
  });

  it("does not cache status-only kindx managers", async () => {
    const agentId = "status-agent";
    const cfg = createKindxCfg(agentId);
    ...
    expect(createKindxManagerMock).toHaveBeenCalledTimes(2);
    // eslint-disable-next-line @typescript-eslint/unbound-method
    expect(createKindxManagerMock).toHaveBeenNthCalledWith(
      1,
  ...
  it("falls back to builtin search when kindx fails with sqlite busy", async () => {
    const retryAgentId = "retry-agent-busy";
    const { manager: firstManager } = await createFailedKindxSearchHarness({
      agentId: retryAgentId,
      errorMessage: "kindx index busy while reading results: SQLITE_BUSY: database is locked",
    });
  ...
  it("keeps original kindx error when fallback manager initialization fails", async () => {
    ...
    await expect(firstManager.search("hello")).rejects.toThrow("kindx query failed");
  });

  it("closes cached managers on global teardown", async () => {
    const cfg = createKindxCfg("teardown-agent");
  ...
  it("closes builtin index managers on teardown after runtime is loaded", async () => {
    ...
    const { manager } = await createFailedKindxSearchHarness({
      agentId: retryAgentId,
      errorMessage: "kindx query failed",
    });

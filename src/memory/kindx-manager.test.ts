...
function createMockChild(params?: { autoClose?: boolean; closeDelayMs?: number }): MockChild {
  ...
  child.kill = () => {
    // Let timeout rejection win in tests that simulate hung KINDX commands.
  };
...
import { KindxMemoryManager } from "./kindx-manager.js";
import { requireNodeSqlite } from "./sqlite.js";

const spawnMock = mockedSpawn as unknown as Mock;

describe("KindxMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: OpenClawConfig }) {
    ...
    const manager = await KindxMemoryManager.create({
      cfg: cfgToUse,
      agentId,
      resolved,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kindx-manager-test-fixtures-"));
  });
  ...
  it("debounces back-to-back sync calls", async () => {
    ...
    (manager as unknown as { lastUpdateAt: number | null }).lastUpdateAt =
      Date.now() - (resolved.kindx?.update.debounceMs ?? 0) - 10;

    await manager.sync({ reason: "after-wait" });
    // `search` mode does not require kindx embed side effects.
  ...
  it("skips kindx command side effects in status mode initialization", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("can be configured to block startup on boot update", async () => {
    ...
    const createPromise = KindxMemoryManager.create({ cfg, agentId, resolved, mode: "full" });
  ...
  it("rebinds sessions collection when existing collection path targets another agent", async () => {
    ...
    const wrongSessionsPath = path.join(stateDir, "agents", agentId, "kindx", "sessions");
    spawnMock.mockImplementation((_cmd: string, args: string[]) => {
      if (args[0] === "collection" && args[1] === "list") {
        const child = createMockChild({ autoClose: false });
        emitAndClose(
    ...
    expect(addSessions?.[2]).toBe(path.join(stateDir, "agents", devAgentId, "kindx", "sessions"));
  });

  it("avoids destructive rebind when kindx only reports collection names", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("warns instead of silently succeeding when add conflict metadata is unavailable", async () => {
    ...
    expect(logWarnMock).toHaveBeenCalledWith(
      expect.stringContaining("kindx collection add skipped for workspace-main"),
    );
  });
      ...
      if (args[0] === "collection" && args[1] === "list") {
        ...
        emitAndClose(
          ...
          [
            "Collections (3):",
            "",
            "memory-root (kindx://memory-root/)",
            "  Pattern:  MEMORY.md",
            "",
            "memory-alt (kindx://memory-alt/)",
            "  Pattern:  memory.md",
            "",
            "memory-dir (kindx://memory-dir/)",
            "  Pattern:  **/*.md",
            "",
          ].join("\n"),
        );
  ...
  it("does not migrate unscoped collections when listed metadata differs", async () => {
    ...
    expect(logDebugMock).toHaveBeenCalledWith(
      expect.stringContaining("kindx legacy collection migration skipped for memory-root"),
    );
  });

  it("times out kindx update during sync when configured", async () => {
    vi.useFakeTimers();
    cfg = {
      ...cfg,
      memory: {
    ...
    const resolved = resolveMemoryBackendConfig({ cfg, agentId });
    const createPromise = KindxMemoryManager.create({ cfg, agentId, resolved, mode: "status" });
    ...
    const rejected = expect(syncPromise).rejects.toThrow("kindx update timed out after 20ms");
    await vi.advanceTimersByTimeAsync(20);
    await rejected;
    await manager.close();
  });

  it("rebuilds managed collections once when kindx update fails with null-byte ENOTDIR", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("rebuilds managed collections once when kindx update hits duplicate document constraint", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("does not rebuild collections for generic kindx update failures", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("uses configured kindx search mode command", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
    ...
    const { manager, resolved } = await createManager();
    const maxResults = resolved.kindx?.limits.maxResults;
    if (!maxResults) {
      throw new Error("kindx maxResults missing");
    }
    ...
    expect(searchCall?.[1]).toEqual([
      ...
      "--json",
      "-n",
      String(resolved.kindx?.limits.maxResults),
      "-c",
      "workspace-main",
    ]);
    expect(
  ...
  it("resolves bare kindx command to a Windows-compatible spawn invocation", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    try {
      const { manager } = await createManager({ mode: "status" });
      await manager.sync({ reason: "manual" });

      const kindxCalls = spawnMock.mock.calls.filter((call: unknown[]) => {
      ...
      expect(kindxCalls.length).toBeGreaterThan(0);
      for (const call of kindxCalls) {
        const command = String(call[0]);
        const options = call[2] as { shell?: boolean } | undefined;
        if (/(^|[\\/])kindx(?:\.cmd)?$/i.test(command)) {
          // Wrapper unresolved: keep `.cmd` and use shell for PATHEXT lookup.
          expect(command.toLowerCase().endsWith("kindx.cmd")).toBe(true);
  ...
  it("normalizes mixed Han-script BM25 queries before kindx search", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
    ...
    await expect(
      manager.search("記憶系統升級 KINDX", { sessionKey: "agent:main:slack:dm:u123" }),
    ).resolves.toEqual([]);
  ...
  it("keeps original Han queries in kindx query mode", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
    ...
    expect(queryCall?.[1]?.[1]).toBe("記憶系統升級 KINDX");
    await manager.close();
  });

  it("retries search with kindx query when configured mode rejects flags", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("scopes kindx queries to managed collections", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("runs kindx query per collection when query mode has multiple collection filters", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("runs kindx searches via mcporter and warns when startDaemon=false", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
        kindx: {
          ...
          update: { interval: "0s", debounceMs: 60_000, onBoot: false },
          paths: [{ path: workspaceDir, pattern: "**/*.md", name: "workspace" }],
          mcporter: { enabled: true, serverName: "kindx", startDaemon: false },
        },
      },
    } as OpenClawConfig;
  ...
  it("passes manager-scoped XDG env to mcporter commands", async () => {
    ...
    expect(normalizePath(spawnOpts?.env?.XDG_CONFIG_HOME)).toContain(
      "/agents/main/kindx/xdg-config",
    );
    expect(normalizePath(spawnOpts?.env?.XDG_CACHE_HOME)).toContain("/agents/main/kindx/xdg-cache");
    ...
    cfg = {
      ...
      memory: {
        ...
        kindx: {
          ...
          update: { interval: "0s", debounceMs: 60_000, onBoot: false },
          paths: [{ path: workspaceDir, pattern: "**/*.md", name: "workspace" }],
          mcporter: { enabled: true, serverName: "kindx", startDaemon: true },
        },
      },
    } as OpenClawConfig;
  ...
  it("logs and continues when kindx embed times out", async () => {
    vi.useFakeTimers();
    cfg = {
      ...cfg,
      memory: {
  ...
  it("skips kindx embed in search mode even for forced sync", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("retries boot update when kindx reports a retryable lock error", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("succeeds on kindx update even when stdout exceeds the output cap", async () => {
    // Regression test for #24966: large indexes produce >200K chars of stdout
    // during `kindx update`, which used to fail with "produced too much output".
    ...
    const { manager } = await createManager({ mode: "status" });
    // sync triggers runKindxUpdateOnce -> runKindx(["update"], { discardOutput: true })
    await expect(manager.sync({ reason: "manual" })).resolves.toBeUndefined();
    await manager.close();
  });
  ...
  it("logs when kindx scope denies search", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
    ...
    expect(logWarnMock).toHaveBeenCalledWith(
      expect.stringContaining("kindx search denied by scope"),
    );
  ...
  it("blocks non-markdown or symlink reads for kindx paths", async () => {
    const { manager } = await createManager();

    const textPath = path.join(workspaceDir, "secret.txt");
    await fs.writeFile(textPath, "nope", "utf-8");
    await expect(manager.readFile({ relPath: "kindx/workspace-main/secret.txt" })).rejects.toThrow(
      "path required",
    );
    ...
    await expect(manager.readFile({ relPath: "kindx/workspace-main/link.md" })).rejects.toThrow(
      "path required",
    );
  ...
  it("returns empty text when kindx files are missing before or during read", async () => {
    const relPath = "kindx-window.md";
  ...
  it("reuses exported session markdown files when inputs are unchanged", async () => {
    ...
    const exportFile = path.join(stateDir, "agents", agentId, "kindx", "sessions", "session-1.md");
    await fs.writeFile(
      sessionFile,
    ...
    const cases = [
      {
        ...
        run: async (manager: KindxMemoryManager) => {
          const inner = manager as unknown as {
            db: {
              prepare: () => {
                all: () => never;
          ...
          await expect(inner.resolveDocLocation("abc123")).rejects.toThrow(
            "kindx index busy while reading results",
          );
        },
      },
      {
        ...
        run: async (manager: KindxMemoryManager) => {
          ...
          ).rejects.toThrow("kindx index busy while reading results");
        },
      },
    ] as const;
  ...
  it("prefers exact docid match before prefix fallback for kindx document lookups", async () => {
  ...
  it("prefers collection hint when resolving duplicate kindx document hashes", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
  ...
  it("resolves search hits when kindx returns kindx:// file URIs without docid", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
    ...
    spawnMock.mockImplementation((_cmd: string, args: string[]) => {
      if (args[0] === "search") {
        ...
        emitAndClose(
          ...
          JSON.stringify([
            {
              file: "kindx://workspace-main/notes/welcome.md",
              score: 0.71,
              snippet: "@@ -4,1\ntoken unlock",
            },
          ]),
  ...
  it("preserves multi-collection kindx search hits when results only include file URIs", async () => {
    cfg = {
      ...cfg,
      memory: {
        backend: "kindx",
    ...
    spawnMock.mockImplementation((_cmd: string, args: string[]) => {
      if (args[0] === "search" && args.includes("workspace-main")) {
        ...
        emitAndClose(
          ...
          JSON.stringify([
            {
              file: "kindx://workspace-main/memory/facts.md",
              score: 0.8,
              snippet: "@@ -2,1\nworkspace fact",
            },
          ]),
      ...
      if (args[0] === "search" && args.includes("notes-main")) {
        ...
        emitAndClose(
          ...
          JSON.stringify([
            {
              file: "kindx://notes-main/guide.md",
              score: 0.7,
              snippet: "@@ -1,1\nnotes guide",
            },
          ]),
  ...
  it("errors when kindx output exceeds command output safety cap", async () => {
  ...
  it("throws when stdout is empty without the no-results marker", async () => {
    ...
    await expect(
      manager.search("missing", { sessionKey: "agent:main:slack:dm:u123" }),
    ).rejects.toThrow(/kindx query returned invalid JSON/);
    await manager.close();
  });

  it("sets busy_timeout on kindx sqlite connections", async () => {
    const { manager } = await createManager();
  ...
  describe("model cache symlink", () => {
    ...
    beforeEach(async () => {
      ...
      defaultModelsDir = path.join(fakeCacheHome, "kindx", "models");
        ...
        },
        {
          ...
          assert: async () => {
            ...
            expect(logWarnMock).not.toHaveBeenCalledWith(
              expect.stringContaining("failed to symlink kindx models directory"),
            );
          },
        },
      ];

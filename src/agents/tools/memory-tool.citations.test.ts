...
describe("memory search citations", () => {
  ...
  it("clamps decorated snippets to kindx injected budget", async () => {
    setMemoryBackend("kindx");
    const cfg = asOpenClawConfig({
      memory: { citations: "on", backend: "kindx", kindx: { limits: { maxInjectedChars: 20 } } },
      agents: { list: [{ id: "main", default: true }] },
    });
    const tool = createMemorySearchTool({ config: cfg });
    if (!tool) {
    ...
    const result = await tool.execute("call_citations_kindx", { query: "notes" });
    const details = result.details as { results: Array<{ snippet: string; citation?: string }> };

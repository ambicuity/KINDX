...
export function attachMediaRoutes(
  ...
  const mediaGetHandler = createMediaGetHandler({ mediaDir, ttlMs });

  app.get("/media/:id", mediaGetHandler);
...
export function createMediaGetHandler(params: { mediaDir: string; ttlMs: number }) {
  return async (req: { params: { id: string } }, res: Response) => {
    ...
    try {
      const {
        ...
        rootDir: params.mediaDir,
        relativePath: id,
        maxBytes: MAX_MEDIA_BYTES,
      });
      if (Date.now() - stat.mtimeMs > params.ttlMs) {
...
export async function startMediaServer(
  ...
  const requestedHost = listenHost?.trim();
  const hosts = requestedHost ? [requestedHost] : ["127.0.0.1", "localhost", "0.0.0.0"];
  ...
  for (const host of hosts) {
    try {
      ...
      const canRetry =
        !requestedHost && (code === "EPERM" || code === "EADDRNOTAVAIL" || code === "EACCES");
      if (!canRetry) {
        throw err;
      }
    }

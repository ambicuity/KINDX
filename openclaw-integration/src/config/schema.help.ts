...
export const FIELD_HELP: Record<string, string> = {
  ...
  "memory.backend":
    'Selects the global memory engine: "builtin" uses OpenClaw memory internals, while "kindx" uses the KINDX sidecar pipeline. Keep "builtin" unless you intentionally operate KINDX.',
  "memory.citations":
  ...
  "memory.kindx.command":
    "Sets the executable path for the `kindx` binary used by the KINDX backend (default: resolved from PATH). Use an explicit absolute path when multiple kindx installs exist or PATH differs across environments.",
  "memory.kindx.mcporter":
    "Routes KINDX work through mcporter (MCP runtime) instead of spawning `kindx` for each call. Use this when cold starts are expensive on large models; keep direct process mode for simpler local setups.",
  "memory.kindx.mcporter.enabled":
    "Routes KINDX through an mcporter daemon instead of spawning kindx per request, reducing cold-start overhead for larger models. Keep disabled unless mcporter is installed and configured.",
  "memory.kindx.mcporter.serverName":
    "Names the mcporter server target used for KINDX calls (default: kindx). Change only when your mcporter setup uses a custom server name for kindx mcp keep-alive.",
  "memory.kindx.mcporter.startDaemon":
    "Automatically starts the mcporter daemon when mcporter-backed KINDX mode is enabled (default: true). Keep enabled unless process lifecycle is managed externally by your service supervisor.",
  "memory.kindx.searchMode":
    'Selects the KINDX retrieval path: "query" uses standard query flow, "search" uses search-oriented retrieval, and "vsearch" emphasizes vector retrieval. Keep default unless tuning relevance quality.',
  "memory.kindx.includeDefaultMemory":
    "Automatically indexes default memory files (MEMORY.md and memory/**/*.md) into KINDX collections. Keep enabled unless you want indexing controlled only through explicit custom paths.",
  "memory.kindx.paths":
    "Adds custom directories or files to include in KINDX indexing, each with an optional name and glob pattern. Use this for project-specific knowledge locations that are outside default memory paths.",
  "memory.kindx.paths.path":
    "Defines the root location KINDX should scan, using an absolute path or `~`-relative path. Use stable directories so collection identity does not drift across environments.",
  "memory.kindx.paths.pattern":
  ...
  "memory.kindx.sessions.enabled":
    "Indexes session transcripts into KINDX so recall can include prior conversation content (experimental, default: false). Enable only when transcript memory is required and you accept larger index churn.",
  "memory.kindx.sessions.exportDir":
    "Overrides where sanitized session exports are written before KINDX indexing. Use this when default state storage is constrained or when exports must land on a managed volume.",
  "memory.kindx.sessions.retentionDays":
  ...
  "memory.kindx.update.interval":
    "Sets how often KINDX refreshes indexes from source content (duration string, default: 5m). Shorter intervals improve freshness but increase background CPU and I/O.",
  "memory.kindx.update.debounceMs":
    "Sets the minimum delay between consecutive KINDX refresh attempts in milliseconds (default: 15000). Increase this if frequent file changes cause update thrash or unnecessary background load.",
  "memory.kindx.update.onBoot":
    "Runs an initial KINDX update once during gateway startup (default: true). Keep enabled so recall starts from a fresh baseline; disable only when startup speed is more important than immediate freshness.",
  "memory.kindx.update.waitForBootSync":
    "Blocks startup completion until the initial boot-time KINDX sync finishes (default: false). Enable when you need fully up-to-date recall before serving traffic, and keep off for faster boot.",
  "memory.kindx.update.embedInterval":
    "Sets how often KINDX recomputes embeddings (duration string, default: 60m; set 0 to disable periodic embeds). Lower intervals improve freshness but increase embedding workload and cost.",
  "memory.kindx.update.commandTimeoutMs":
    "Sets timeout for KINDX maintenance commands such as collection list/add in milliseconds (default: 30000). Increase when running on slower disks or remote filesystems that delay command completion.",
  "memory.kindx.update.updateTimeoutMs":
    "Sets maximum runtime for each `kindx update` cycle in milliseconds (default: 120000). Raise this for larger collections; lower it when you want quicker failure detection in automation.",
  "memory.kindx.update.embedTimeoutMs":
    "Sets maximum runtime for each `kindx embed` cycle in milliseconds (default: 120000). Increase for heavier embedding workloads or slower hardware, and lower to fail fast under tight SLAs.",
  "memory.kindx.limits.maxResults":
    "Limits how many KINDX hits are returned into the agent loop for each recall request (default: 6). Increase for broader recall context, or lower to keep prompts tighter and faster.",
  "memory.kindx.limits.maxSnippetChars":
    "Caps per-result snippet length extracted from KINDX hits in characters (default: 700). Lower this when prompts bloat quickly, and raise only if answers consistently miss key details.",
  "memory.kindx.limits.maxInjectedChars":
    "Caps how much KINDX text can be injected into one turn across all hits. Use lower values to control prompt bloat and latency; raise only when context is consistently truncated.",
  "memory.kindx.limits.timeoutMs":
    "Sets per-query KINDX search timeout in milliseconds (default: 4000). Increase for larger indexes or slower environments, and lower to keep request latency bounded.",
  "memory.kindx.scope":
    "Defines which sessions/channels are eligible for KINDX recall using session.sendPolicy-style rules. Keep default direct-only scope unless you intentionally want cross-chat memory sharing.",

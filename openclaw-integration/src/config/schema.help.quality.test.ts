...
const ENUM_EXPECTATIONS: Record<string, string[]> = {
  ...
  "memory.backend": ['"builtin"', '"kindx"'],
  "memory.kindx.searchMode": ['"query"', '"search"', '"vsearch"'],
...
describe("config help copy quality", () => {
  ...
  it("includes concrete examples on path and interval fields", () => {
    expect(FIELD_HELP["memory.kindx.paths.pattern"].includes("**/*.md")).toBe(true);
    expect(FIELD_HELP["memory.kindx.update.interval"].includes("5m")).toBe(true);
    expect(FIELD_HELP["memory.kindx.update.embedInterval"].includes("60m")).toBe(true);

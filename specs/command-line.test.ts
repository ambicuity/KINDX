 ...
 *
 * Tests all kindx CLI commands using a temporary test database via INDEX_PATH.
 * These tests spawn actual kindx processes to verify end-to-end functionality.
 */
...
This is a test project for KINDX CLI testing.
...
`
    ...
    const before = await runQmd(["get", "kindx://empty-check/only.md"], { dbPath, configDir });
...
describe("CLI Context Management", () => {
  ...
  test("add context to virtual path", async () => {
    ...
    const { stdout, exitCode } = await runQmd([
      "context",
      "add",
      "kindx://fixtures/notes",
      "Context for notes subdirectory",
    ], { dbPath: localDbPath });
    expect(exitCode).toBe(0);
    expect(stdout).toContain("✓ Added context for: kindx://fixtures/notes");
  });
  ...
  test("remove virtual path context", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toContain("✓ Removed context for: kindx://fixtures/notes");
  });
...
describe("CLI ls Command", () => {
  ...
  test("lists all collections", async () => {
    ...
    expect(stdout).toContain("kindx://fixtures/");
  });
  ...
  test("lists files in a collection", async () => {
    ...
    expect(stdout).toContain("kindx://fixtures/readme.md");
    expect(stdout).toContain("kindx://fixtures/notes/meeting.md");
  });
  ...
  test("lists files with path prefix", async () => {
    ...
    expect(exitCode).toBe(0);
    expect(stdout).toContain("kindx://fixtures/notes/meeting.md");
    expect(stdout).toContain("kindx://fixtures/notes/ideas.md");
    // Should not include files outside the prefix (handelize converts to lowercase)
    expect(stdout).not.toContain("kindx://fixtures/readme.md");
  });

  test("lists files with virtual path", async () => {
    const { stdout, exitCode } = await runQmd(["ls", "kindx://fixtures/docs"], { dbPath: localDbPath });
    expect(exitCode).toBe(0);
    expect(stdout).toContain("kindx://fixtures/docs/api.md");
  });
...
describe("CLI Collection Commands", () => {
  ...
  test("renames a collection", async () => {
    // First verify the collection exists
    const { stdout: listBefore } = await runQmd(["collection", "list"], { dbPath: localDbPath });
    expect(listBefore).toContain("kindx://fixtures/");
    ...
    expect(stdout).toContain("kindx://fixtures/");
    expect(stdout).toContain("kindx://my-fixtures/");

    // Verify the new name exists and old name is gone
    const { stdout: listAfter } = await runQmd(["collection", "list"], { dbPath: localDbPath });
    expect(listAfter).toContain("kindx://my-fixtures/");
    expect(listAfter).not.toContain("kindx://fixtures/"); // Old collection should not appear
  });
  ...
  test("handles renaming to existing collection name", async () => {
    ...
    const tempDir = await mkdtemp(join(tmpdir(), "kindx-second-"));
    ...
    expect(listBoth).toContain("kindx://fixtures/");
    expect(listBoth).toContain("kindx://second/");
...
// =============================================================================
// Output Format Tests - kindx:// URIs, context, and docid
// =============================================================================
...
describe("search output formats", () => {
  ...
  beforeAll(async () => {
    ...
    // Add context
    await runQmd(["context", "add", `kindx://${collName}/`, "Test fixtures for QMD"], { dbPath: localDbPath, configDir: localConfigDir });
  });

  test("search --json includes kindx:// path, docid, and context", async () => {
    ...
    expect(result.file).toMatch(new RegExp(`^kindx://${collName}/`));
  ...
  test("search --files includes kindx:// path, docid, and context", async () => {
    const { stdout, exitCode } = await runQmd(["search", "test", "--files", "-n", "1"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);

    // Format: #docid,score,kindx://collection/path,"context"
    expect(stdout).toMatch(new RegExp(`^#[a-f0-9]{6},[\\d.]+,kindx://${collName}/`, "m"));
  ...
  test("search --csv includes kindx:// path, docid, and context", async () => {
    ...
    // Data rows should have kindx:// paths and context
    expect(stdout).toMatch(new RegExp(`#[a-f0-9]{6},[\\d.]+,kindx://${collName}/`));
    expect(stdout).toContain("Test fixtures for QMD");
    // Ensure no full filesystem paths
    expect(stdout).not.toMatch(/\/Users\//);
    expect(stdout).not.toMatch(/\/home\//);
  ...
  test("search --xml includes kindx:// path, docid, and context", async () => {
    const { stdout, exitCode } = await runQmd(["search", "test", "--xml", "-n", "1"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);

    expect(stdout).toMatch(new RegExp(`<file docid="#[a-f0-9]{6}" name="kindx://${collName}/`));
    expect(stdout).toContain('context="Test fixtures for QMD"');
    // Ensure no full filesystem paths
    expect(stdout).not.toMatch(/\/Users\//);
    expect(stdout).not.toMatch(/\/home\//);
  ...
  test("search default CLI format includes kindx:// path, docid, and context", async () => {
    const { stdout, exitCode } = await runQmd(["search", "test", "-n", "1"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);

    // First line should have kindx:// path and docid
    expect(stdout).toMatch(new RegExp(`^kindx://${collName}/.*#[a-f0-9]{6}`, "m"));
    expect(stdout).toContain("Context: Test fixtures for QMD");
    // Ensure no full filesystem paths
    expect(stdout).not.toMatch(/\/Users\//);
    expect(stdout).not.toMatch(/\/home\//);
...
describe("get command path normalization", () => {
  ...
  test("get with kindx://collection/path format", async () => {
    const { stdout, exitCode } = await runQmd(["get", `kindx://${collName}/test1.md`, "-l", "3"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);
  ...
  test("get with kindx:////collection/path format (extra slashes)", async () => {
    const { stdout, exitCode } = await runQmd(["get", `kindx:////${collName}/test1.md`, "-l", "3"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);
    expect(stdout).toContain("Test Document 1");
  });
  ...
  test("get with kindx://path:line format", async () => {
    const { stdout, exitCode } = await runQmd(["get", `kindx://${collName}/test1.md:3`, "-l", "2"], { dbPath: localDbPath, configDir: localConfigDir });
    expect(exitCode).toBe(0);
    // Should start from line 3, not line 1
    expect(stdout).not.toMatch(/^# Test Document 1$/m);
  });
...
describe("status and collection list hide filesystem paths", () => {
  ...
  test("status does not show full filesystem paths", async () => {
    ...
    // Should show kindx:// URIs
    expect(stdout).toContain(`kindx://${collName}/`);
...
describe("mcp http daemon", () => {
  ...
  let daemonCacheDir: string; // XDG_CACHE_HOME value (the kindx/ subdir is created automatically)
  ...
  function pidPath(): string {
    return join(daemonCacheDir, "kindx", "mcp.pid");
  }

  /** Run kindx with test-isolated env (cache, db, config) */
  ...
  beforeAll(async () => {
    daemonTestDir = await mkdtemp(join(tmpdir(), "kindx-daemon-test-"));
    ...
    await mkdir(join(daemonCacheDir, "kindx"), { recursive: true });

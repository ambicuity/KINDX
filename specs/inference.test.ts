...
describe("LlamaCpp expand context size config", () => {
  ...
  test("uses default expand context size when no config or env is set", () => {
    const prev = process.env.KINDX_EXPAND_CONTEXT_SIZE;
    delete process.env.KINDX_EXPAND_CONTEXT_SIZE;
    try {
      const llm = new LlamaCpp({}) as any;
      expect(llm.expandContextSize).toBe(defaultExpandContextSize);
    } finally {
      if (prev === undefined) delete process.env.KINDX_EXPAND_CONTEXT_SIZE;
      else process.env.KINDX_EXPAND_CONTEXT_SIZE = prev;
    }
  });

  test("uses KINDX_EXPAND_CONTEXT_SIZE when set to a positive integer", () => {
    const prev = process.env.KINDX_EXPAND_CONTEXT_SIZE;
    process.env.KINDX_EXPAND_CONTEXT_SIZE = "3072";
    try {
      const llm = new LlamaCpp({}) as any;
  ...
  test("config value overrides KINDX_EXPAND_CONTEXT_SIZE", () => {
    const prev = process.env.KINDX_EXPAND_CONTEXT_SIZE;
    process.env.KINDX_EXPAND_CONTEXT_SIZE = "4096";
    try {
  ...
  test("falls back to default and warns when KINDX_EXPAND_CONTEXT_SIZE is invalid", () => {
    const prev = process.env.KINDX_EXPAND_CONTEXT_SIZE;
    process.env.KINDX_EXPAND_CONTEXT_SIZE = "bad";
    const stderrSpy = vi.spyOn(process.stderr, "write").mockReturnValue(true);
    try {
      const llm = new LlamaCpp({}) as any;
      expect(llm.expandContextSize).toBe(defaultExpandContextSize);
      expect(stderrSpy).toHaveBeenCalled();
      expect(String(stderrSpy.mock.calls[0]?.[0] || "")).toContain("KINDX_EXPAND_CONTEXT_SIZE");
    } finally {
...
describe.skipIf(!!process.env.CI)("LlamaCpp Integration", () => {
  ...
  describe("embedBatch", () => {
    ...
    test("handles concurrent embedBatch calls on fresh instance without race condition", async () => {
      ...
      //
      // See: https://github.com/ambicuity/KINDX/pull/54
      //
